...
router.post('/', [authenticateToken, isAdmin, upload.single('image')], async (req, res) => {
  const { name, slug, description, gender = 'unisex', display_order = 0, is_active = 1 } = req.body;
  ...
  try {
    ...
    const result = await dbAsync.run(
      'INSERT INTO categories (name, slug, description, image_url, gender, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [name, slug, description, imageUrl, gender, display_order, is_active]
    );
...
router.put('/:id', [authenticateToken, isAdmin, upload.single('image')], async (req, res) => {
  const { id } = req.params;
  const { name, slug, description, gender, display_order, is_active } = req.body;
  ...
  try {
    ...
    await dbAsync.run(
      'UPDATE categories SET name = ?, slug = ?, description = ?, image_url = ?, gender = ?, display_order = ?, is_active = ? WHERE id = ?',
      [name, slug, description, imageUrl, gender || 'unisex', display_order, is_active, id]
    );
