...
router.get('/', optionalAuth, async (req, res) => {
  const { category, gender, featured, search, sort = 'display_order', order = 'ASC', limit = 20, offset = 0 } = req.query;
  ...
  try {
    ...
    const allowedSorts = ['created_at', 'price', 'name', 'views', 'display_order'];
...
router.post('/', [authenticateToken, isAdmin], async (req, res) => {
  ...
  try {
    ...
    if (display_order === undefined || display_order === null) {
      const maxOrder = await dbAsync.get('SELECT MAX(display_order) as max_order FROM products');
    ...
    `, [
      ...
      weight || null, gender || 'unisex', is_featured ? 1 : 0, display_order
    ]);
...
router.put('/:id', [authenticateToken, isAdmin], async (req, res) => {
  let {
    category_id, name, slug, description, price, discount_price,
    stock_quantity, sku, material, weight, gender, is_featured, is_active, display_order
  } = req.body;
  ...
  try {
    ...
    `, [
      category_id, name, slug, description || '', price, discount_price || null,
      stock_quantity, sku || null, material, weight || null, gender || 'unisex',
      is_featured ? 1 : 0, is_active ? 1 : 0, display_order || 0, req.params.id
    ]);
...
router.put('/reorder', [authenticateToken, isAdmin], async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: { message: 'Sıralama güncellenirken hata oluştu' } });
  }
});
